export default function ProfileField({ label, note, value, onChange, placeholder, required }) {
  return (
    <label className="block">
      ...
      <input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="mt-3 h-12 w-full border border-[var(--a-line)] bg-[var(--a-panel)] px-4 text-[14px] text-[var(--a-ink)] outline-none transition placeholder:text-[var(--a-ink)]/24 focus:border-[var(--a-muted)]/60" />
    </label>
  )
}
