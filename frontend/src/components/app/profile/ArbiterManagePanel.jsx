...
export default function ArbiterManagePanel({ wallet }) {
  ...
  return (
    <div className="border border-[var(--a-line)] bg-[var(--a-surface)] p-5 sm:p-6">
      ...
      <h3 className="mt-4 text-[28px] font-medium leading-[1] tracking-[-0.06em] text-[var(--a-ink)]">Owner controls dispute access.</h3>
      <p className="mt-3 text-[13px] leading-[1.65] text-[var(--a-muted)]">Active arbiters can resolve disputed rooms on-chain. Disputed rooms are already frozen until an owner or active arbiter resolves/splits them.</p>
      ...
      {isOwner ? (
        ...
        <div className="mt-5 border border-[var(--a-line)] bg-[var(--a-panel)] p-4 text-[13px] leading-[1.6] text-[var(--a-muted)]">Only the contract owner can add or remove arbiters.</div>
      )}
      ...
      <div className="mt-5 grid gap-3 border-t border-[var(--a-line)] pt-5">
        ...
        {checkedArbiter && (
          <div className={`border px-4 py-3 text-[13px] ${checkedArbiter.active ? 'border-[#8f9a88]/28 bg-[#8f9a88]/10 text-[#8f9a88]' : 'border-[var(--a-line)] bg-[var(--a-panel)] text-[var(--a-muted)]'}`}>
      ...
      {status && (
        <div className={`mt-5 border px-4 py-3 text-[13px] leading-[1.55] ${status.type === 'ok' ? 'border-[#8f9a88]/28 bg-[#8f9a88]/10 text-[#8f9a88]' : status.type === 'err' ? 'border-[#b87333]/35 bg-[#b87333]/10 text-[#b87333]' : 'border-[var(--a-muted)]/24 bg-[var(--a-muted)]/[0.07] text-[var(--a-muted)]'}`}>{status.msg}</div>
      )}
    </div>
  )
}
