export default function ProfileSetupIntro({ wallet }) {
  return (
    <div>
      <div className="font-mono text-[10px] uppercase tracking-[0.28em] text-[var(--a-muted,#a3a3a3)]">Profile setup</div>
      ...
      <div className="mt-8 border-l border-[var(--a-line)] pl-4 font-mono text-[10px] uppercase leading-[1.9] tracking-[0.16em] text-[var(--a-ink,#fafafa)]/46">
        Wallet<br />
        <span className="text-[var(--a-ink,#fafafa)]">{wallet.address.slice(0, 8)}…{wallet.address.slice(-6)}</span>
      </div>
    </div>
  )
}
