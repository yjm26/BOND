export default function AppGateCopy({ connecting, connectError, onConnect }) {
  return (
    <div className="relative z-10 mx-auto w-full max-w-[640px] text-center">
      ...
      <h1 className="mx-auto mt-5 max-w-[18ch] text-[clamp(40px,6.2vw,72px)] font-medium leading-[0.92] tracking-[-0.07em] text-[#ede9df]">
      ...
      <button
        ...
        onClick={onConnect}
        disabled={connecting}
        className="mt-8 inline-flex h-12 items-center justify-center border border-[#ede9df] bg-[#ede9df] px-7 font-mono text-[12px] font-semibold uppercase tracking-[0.16em] text-[#20201f] transition duration-160 ease-out hover:bg-transparent hover:text-[#ede9df] active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-50"
      >
        {connecting ? 'Connecting…' : 'Connect wallet'}
      </button>
      {connectError && (
        <div className="mx-auto mt-5 max-w-[480px] border border-[#8d2f2f]/40 bg-[#8d2f2f]/10 px-4 py-3 text-left text-[13px] leading-[1.55] text-[#f0c2b8]">
          {connectError}
        </div>
      )}
      <div className="mt-8 font-mono text-[10px] uppercase tracking-[0.2em] text-[#ede9df]/34">
        Arc Testnet
      </div>
    </div>
  )
