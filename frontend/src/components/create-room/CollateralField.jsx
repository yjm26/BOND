  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <label className={`flex items-center gap-2 text-[12px] text-[var(--a-muted,#a3a3a3)] ${fromMarket ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}>
      ...
      {!noCollateral ? (
        <CreateRoomField suffix="USDC" disabled={fromMarket} note={creatorIsSeller ? 'If you are seller, this can be locked when the room is created.' : 'Seller must lock this when joining. Lost if they fail delivery.'}>
          <input className="h-12 w-full border border-[var(--a-line)] bg-[var(--a-panel,#0a0a0a)] px-4 pr-16 text-[14px] text-[var(--a-ink,#fafafa)] outline-none transition placeholder:text-[var(--a-ink,#fafafa)]/24 focus:border-[var(--a-muted,#a3a3a3)]/60 disabled:cursor-not-allowed disabled:opacity-60" type="number" placeholder="0.00" min="0" step="0.01" value={collateral} onChange={(event) => onCollateralChange(event.target.value)} readOnly={fromMarket} disabled={fromMarket} />
        </CreateRoomField>
      ) : (
        <p className="border border-[var(--a-line)] bg-[var(--a-panel,#0a0a0a)] px-4 py-3 text-[12px] leading-[1.55] text-[var(--a-muted,#a3a3a3)]">No collateral — the room can still work, but trust relies more on reputation and proof.</p>
      )}
    </div>
  )
}
