  ...
  return (
    <aside className="hidden border border-[var(--a-line)] bg-[var(--a-surface)] p-4 lg:block">
      ...
      <div className="mt-6 space-y-1">
        {actions.map((item) => (
          <Link key={item.label} to={item.to} className={`flex h-10 items-center justify-between border px-3 text-[13px] transition ${item.label === 'Create room' ? 'border-[var(--a-line)] bg-[var(--a-inverse-bg)]/6 text-[var(--a-ink)]' : 'border-transparent text-[var(--a-ink)]/62 hover:border-[var(--a-line)] hover:bg-[var(--a-inverse-bg)]/5 hover:text-[var(--a-ink)]'}`}>
