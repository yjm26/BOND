...
export default function OfferModal({ listing, wallet, onClose, onSubmitted }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white dark:bg-[#111111] rounded-lg border border-zinc-200 dark:border-white/10 shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSubmit} disabled={!price || submitting} className={`flex-1 py-2.5 rounded text-[13px] font-medium transition disabled:opacity-40 ${isAccepting ? 'bg-zinc-900 dark:bg-white text-white dark:text-[#0a0a0a] hover:bg-zinc-800 dark:hover:bg-gray-200' : 'bg-zinc-900 dark:bg-white text-white dark:text-[#0a0a0a] hover:bg-zinc-800 dark:hover:bg-gray-200'}`}>
