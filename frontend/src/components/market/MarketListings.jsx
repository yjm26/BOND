...
export default function MarketListings({ loading, listings, search, wallet, onOpenDeal, onDelete, onExpand }) {
  if (loading) {
    return <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{Array.from({ length: 10 }).map((_, index) => <div key={index} className="h-[188px] animate-pulse border border-[#ede9df]/12 bg-[#1a1a18]" />)}</div>
  }
