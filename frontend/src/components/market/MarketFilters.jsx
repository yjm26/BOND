...
export default function MarketFilters({ search, sort, filter, onSearchChange, onSortChange, onFilterChange }) {
  return (
    <div className="mb-5 border border-[var(--a-line)] bg-[var(--a-surface)] p-3">
      <div className="grid gap-3 lg:grid-cols-[1fr_220px]">
        <input className="h-12 w-full border border-[var(--a-line)] bg-[var(--a-panel)] px-4 text-[14px] text-[var(--a-ink)] outline-none transition placeholder:text-[var(--a-ink)]/28 focus:border-[var(--a-muted)]/60" placeholder="Search listings…" value={search} onChange={(event) => onSearchChange(event.target.value)} />
        <select className="h-12 w-full border border-[var(--a-line)] bg-[var(--a-panel)] px-3 font-mono text-[11px] uppercase tracking-[0.12em] text-[var(--a-ink)] outline-none transition focus:border-[var(--a-muted)]/60" value={sort} onChange={(event) => onSortChange(event.target.value)}>
