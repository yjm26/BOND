...
export default function RoomPartiesPanel({ room, isCreator, isCounter, creatorRep, counterpartyRep, role }) {
  ...
  return (
    <div className="border border-[var(--a-line)] bg-[var(--a-surface)] p-5">
      <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.22em] text-[var(--a-ink)]/40">Parties</div>
      <div className="grid gap-3">
        ...
        {room.counterparty !== '0x0000000000000000000000000000000000000000' ? (
          ...
          <div className="border border-dashed border-[var(--a-line)] bg-[var(--a-panel)] p-4 text-[13px] text-[var(--a-muted)]">Waiting for counterparty to join…</div>
        )}
      </div>
      <div className="mt-4 border-t border-[var(--a-line)] pt-4 text-[13px] text-[var(--a-muted)]">Your role: <span className="text-[var(--a-ink)]">{role || 'Viewer'}</span></div>
    </div>
  )
}
