export default function RoomMetricCard({ label, value, note, tone = 'default' }) {
  const toneClass = tone === 'success' ? 'text-[#8f9a88]' : tone === 'warning' ? 'text-[var(--a-muted)]' : tone === 'danger' ? 'text-[#b87333]' : 'text-[var(--a-ink)]'
  return (
    <div className="border border-[var(--a-line)] bg-[var(--a-surface)] p-5">
      ...
      {note && <div className="mt-2 text-[12px] leading-[1.55] text-[var(--a-muted)]">{note}</div>}
    </div>
  )
}
