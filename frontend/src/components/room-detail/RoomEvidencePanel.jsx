...
export default function RoomEvidencePanel({ room, evidence }) {
  ...
  return (
    <div className="border border-[#b87333]/30 bg-[var(--a-surface)] p-5">
      <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.22em] text-[#b87333]">Submitted evidence</div>
      <div className="grid gap-3">
        {evidence.map((ev) => (
          <div key={ev.id} className="border border-[var(--a-line)] bg-[var(--a-panel)] p-4">
            ...
            {ev.description && <div className="text-[13px] leading-[1.6] text-[var(--a-muted)]">{ev.description}</div>}
            {ev.evidenceRef && <div className="mt-2 break-all font-mono text-[11px] text-[var(--a-muted)]">{ev.evidenceRef.startsWith('http') ? <a href={ev.evidenceRef} target="_blank" rel="noopener noreferrer">{ev.evidenceRef}</a> : ev.evidenceRef}</div>}
          </div>
        ))}
      </div>
    </div>
