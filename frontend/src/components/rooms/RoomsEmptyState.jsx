...
export default function RoomsEmptyState({ wallet }) {
  return (
    <div className="grid min-h-[280px] place-items-center border border-[var(--a-line)] bg-[var(--a-surface,#111111)] p-8 text-center">
      ...
      {wallet && <Link to="/create" className="mt-6 inline-flex h-11 items-center border border-[var(--a-ink,#fafafa)] bg-[var(--a-inverse-bg,#fafafa)] px-5 font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-[#111111] transition hover:bg-transparent hover:text-[var(--a-ink,#fafafa)]">Create room</Link>}
    </div>
  )
}
