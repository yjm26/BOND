...
export default function ArbiterGate({ role, loadingRole, error }) {
  return (
    <div className="border border-[var(--a-line)] bg-[var(--a-surface,#111111)] p-8 text-center sm:p-10">
      <div className="mx-auto flex h-12 w-12 items-center justify-center border border-[var(--a-line)] bg-[var(--a-panel,#0a0a0a)] font-mono text-[13px] text-[var(--a-muted,#a3a3a3)]">!</div>
      <div className="mt-6 font-mono text-[10px] uppercase tracking-[0.24em] text-[var(--a-muted,#a3a3a3)]">Dispute desk</div>
      <h1 className="mt-4 text-[clamp(34px,5vw,60px)] font-medium leading-[0.94] tracking-[-0.08em] text-[var(--a-ink,#fafafa)]">
        {loadingRole ? 'Checking access.' : 'Arbiter access required.'}
      </h1>
      <p className="mx-auto mt-4 max-w-[560px] text-[14px] leading-[1.7] text-[var(--a-muted,#a3a3a3)]">
      ...
      {!loadingRole && role && <div className="mt-5 font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--a-ink,#fafafa)]/36">Current role: <span className="text-[var(--a-ink,#fafafa)]">{role}</span></div>}
    </div>
  )
}
