...
export default function ArbiterDecisionPanel({ room, role, resolving, txStatus, onResolve, onSplit }) {
  ...
  return (
    <div className="border border-[var(--a-line)] bg-[var(--a-surface)] p-5">
      ...
      <h3 className="mt-4 text-[28px] font-medium leading-[1] tracking-[-0.06em] text-[var(--a-ink)]">Move locked funds.</h3>
      <p className="mt-3 text-[13px] leading-[1.65] text-[var(--a-muted)]">Resolve sends escrow to one side. Split sends the funded amount 50/50 and returns seller collateral to seller. Use only after reviewing proof.</p>
      <div className="mt-5 grid gap-3">
        <button disabled={!canAct || resolving} onClick={() => onResolve(room.buyer)} className="border border-[#8f9a88]/35 bg-[#8f9a88]/10 px-4 py-4 text-left transition hover:bg-[#8f9a88]/15 disabled:cursor-not-allowed disabled:opacity-40">
          ...
          <span className="mt-2 block font-mono text-[12px] text-[var(--a-ink)]">{formatAddress(room.buyer)}</span>
        </button>
        <button disabled={!canAct || resolving} onClick={() => onResolve(room.seller)} className="border border-[var(--a-muted)]/35 bg-[var(--a-muted)]/10 px-4 py-4 text-left transition hover:bg-[var(--a-muted)]/15 disabled:cursor-not-allowed disabled:opacity-40">
          <span className="block font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--a-muted)]">Resolve to seller</span>
          <span className="mt-2 block font-mono text-[12px] text-[var(--a-ink)]">{formatAddress(room.seller)}</span>
        </button>
        <button disabled={!canAct || resolving} onClick={onSplit} className="border border-[var(--a-line)] px-4 py-4 text-left transition hover:border-[var(--a-line-strong)] disabled:cursor-not-allowed disabled:opacity-40">
          <span className="block font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--a-ink)]/64">Split escrow</span>
